...
const DeafultURL = "https://api.github.com/users/rahul06r";
// 
// const PagInationURL = "https://api.github.com/users/kkrypt0nn/repos?page=1&per_page=10";
const PagInationURL = "https://api.github.com/users/rahul06r/repos?page=1&per_page=10";
...
const showRepoDetails = async (url) => {
    ...
    try {
        ...
        if (Array.isArray(repoData) && repoData.length > 0) {
            ...
            console.log('Invalid or empty User Name');
        }
    } catch (error) {
        repoContainer.innerHTML = "<p>Error Fetching Repository Details.\n Please try again later.</p>";
    ...
    try {
        ...
        if (userName) {
            ...
            if (response.ok) {
                ...
                console.error(`Error fetching Status codes: ${response.status}`);
            }
        } else {
...
function updatePagination(pageNumber, data, reposPerPage) {
    ...
    const url = `${APIURL}${data.login}/repos?page=${pageNumber}&per_page=${reposPerPage}&sort=created&direction=desc`;
    showRepoDetails(url);
